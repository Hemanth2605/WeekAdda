...
frontend/
  src/
    pages/                 # Releases.tsx, Cricket.tsx, Blog.tsx, MovieDetail.tsx
...
- **Dynamic sitemap**: `/sitemap.xml` is generated by the Worker (`buildSitemap`) —
  the four static routes plus every current title page (~1,000+ URLs), `lastmod` set
  to the last sweep, so crawlers see daily change. The static `public/sitemap.xml`
  is an unused fallback.
- The Worker stamps route-specific `<title>`/description/canonical into the raw HTML
  (`routeMeta`); the SPA sets matching per-view titles at runtime (`src/seo.ts`).
  Titles are kept within ~65 chars and descriptions within 160 (Bing guidelines).
